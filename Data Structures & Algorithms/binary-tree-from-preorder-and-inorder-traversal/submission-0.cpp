...
public:
    ...
    TreeNode* buildTree(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, map<int, int>& imap){
        ...
        root->left = buildTree(preorder, preStart + 1, preStart + leftNo, inorder, inStart, inRoot - 1, imap);
        root->right = buildTree(preorder, preStart + leftNo + 1, preEnd, inorder, inRoot + 1, inEnd, imap);
